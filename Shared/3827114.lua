...
local m_GetPromiseLibrary = require(script.GetPromiseLibrary)
local m_RbxScriptConnection = require(script.RbxScriptConnection)
local m_Symbol = require(script.Symbol)
local v_u_4, v_u_5 = m_GetPromiseLibrary()
local v_u_6 = m_Symbol("IndicesReference")
local v_u_7 = m_Symbol("LinkToInstanceIndex")
local v_u_8 = {
    ["ClassName"] = "Janitor",
...
function v_u_8.RemoveNoClean(p34, p35) -- name: RemoveNoClean
    -- upvalues: (copy) v_u_6
    local v36 = p34[v_u_6]
    if v36 then
        local v37 = v36[p35]
...
function v_u_8.RemoveList(p38, ...) -- name: RemoveList
    -- upvalues: (copy) v_u_6
    if p38[v_u_6] then
        local v39 = select("#", ...)
        if v39 == 1 then
...
function v_u_8.RemoveListNoClean(p41, ...) -- name: RemoveListNoClean
    -- upvalues: (copy) v_u_6
    local v42 = p41[v_u_6]
    if v42 then
        local v43 = select("#", ...)
...
function v_u_8.Get(p47, p48) -- name: Get
    -- upvalues: (copy) v_u_6
    local v49 = p47[v_u_6]
    if v49 then
        return v49[p48]
...
function v_u_8.GetAll(p50) -- name: GetAll
    -- upvalues: (copy) v_u_6
    local v51 = p50[v_u_6]
    return not v51 and {} or table.freeze(table.clone(v51))
end
function v_u_8.Cleanup(p_u_52) -- name: Cleanup
    -- upvalues: (copy) v_u_6
...
function v_u_8.Destroy(p61) -- name: Destroy
    p61:Cleanup()
end
v_u_8.__call = v_u_8.Cleanup
function v_u_8.LinkToInstance(p_u_62, p63, p64) -- name: LinkToInstance
    -- upvalues: (copy) v_u_7
...
function v_u_8.LegacyLinkToInstance(p_u_66, p67, p68) -- name: LegacyLinkToInstance
    -- upvalues: (copy) v_u_7, (copy) m_RbxScriptConnection
    local v_u_69 = nil
    local v70 = p68 and newproxy(false) or v_u_7
    local v_u_71 = p67.Parent == nil
    local v72 = m_RbxScriptConnection
    local v_u_73 = setmetatable({}, v72)
    local function v75(_, p74) -- name: ChangedFunction
...
function v_u_8.LinkToInstances(p78, ...) -- name: LinkToInstances
    -- upvalues: (copy) v_u_8
    local v79 = v_u_8.new()
    for _, v80 in { ... } do
